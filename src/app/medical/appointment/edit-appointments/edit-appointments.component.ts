...
export class EditAppointmentsComponent {
    ...
    this.appointmentService.listConfig().subscribe((resp: any) => {
      ...
      this.appointmentService
        ...
        .subscribe((resp: any) => {
          ...
          this.name_companion =
            this.appointment_selected.patient.name_companion;
          this.surname_companion =
            this.appointment_selected.patient.surname_companion;

          this.date_appointment = new Date(
            this.appointment_selected.date_appointment
          ).toISOString();
          ...
          this.hour =
            this.appointment_selected.segment_hour.format_segment.hour;
          // this.filtro();
        });
    });
  }
  save() {
    ...
    if (!this.date_appointment || !this.specialitie_id || !this.amount) {
      this.text_validation =
        'LOS CAMPOS SON NECESARIOS (LA FECHA , LA ESPECIALIDAD Y EL TOTAL DE PAGOS)';
      return;
    }
    if (
      new Date(this.date_appointment).getTime() !=
      new Date(this.appointment_selected.date_appointment).getTime()
    ) {
    ...
    const data = {
      ...
      doctor_schedule_join_hour_id: this.selected_segment_hour
        ? this.selected_segment_hour.id
        : this.appointment_selected.doctor_schedule_join_hour_id,
      amount: this.amount,
    };

    this.appointmentService
      .updateAppointment(this.appointment_id, data)
      .subscribe((resp: any) => {
        console.log(resp);
        if (resp.message == 403) {
