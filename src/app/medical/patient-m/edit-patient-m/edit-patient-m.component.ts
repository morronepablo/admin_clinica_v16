...
export class EditPatientMComponent {
  ...
  ngOnInit(): void {
    ...
    this.patientService.showPatient(this.patient_id).subscribe((resp: any) => {
      ...
      this.birth_date = resp.patient.birth_date
        ? new Date(resp.patient.birth_date).toISOString()
        : '';
      ...
      this.relationship_responsible =
        resp.patient.person.relationship_responsible;
  ...
  save() {
    ...
    if (!this.name_companion || !this.surname_companion) {
      this.text_validation =
        'El nombre y apellido del acompañante es obligatorio.';
      return;
    }
    ...
    this.patientService
      .updatePatient(this.patient_id, formData)
      .subscribe((resp: any) => {
        console.log(resp);
